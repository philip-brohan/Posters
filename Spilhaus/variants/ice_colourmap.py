...
ice_cm = matplotlib.colors.ListedColormap(cm_data, name='ice_cm')
...
# Load the Meto SST
sst=iris.load_cube('%s/Posters/Spilhaus/tsurf.nc' % os.getenv('SCRATCH'))
...
# And the orography
orog=iris.load_cube('%s/Posters/Spilhaus/orography.nc' % os.getenv('SCRATCH'))
...
mask=iris.load_cube('%s/Posters/Spilhaus/land_mask.nc' % os.getenv('SCRATCH'))
...
icec.data = numpy.ma.array(icec.data,mask=icec.data<0.15)
