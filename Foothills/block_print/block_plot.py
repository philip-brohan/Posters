...
fig = Figure(
    figsize=(20,2),  # Width, Height (inches)
    dpi=300,
...
axb.set_aspect("auto")
axb.add_patch(
    Rectangle((p_lat_min, bottom), p_lat_max-p_lat_min, top-bottom, facecolor=(0.25, 0.65, 0.95, 1), fill=True, zorder=1,)
)


def plot_layer(lon_idx,colour,decimate_by=None):
    height_series = rdata[:,lon_idx]
    llats = lats[:,lon_idx]
    llons = lons[:,lon_idx]
    (p_lats,p_heights) = project_coordinates(llats,llons,height_series)
    height_series = p_heights[(p_lats>p_lat_min) & (p_lats<p_lat_max)]
    llats = p_lats[(p_lats>p_lat_min) & (p_lats<p_lat_max)]
    if decimate_by is not None: # Lower resolution  -> faster
        height_series = np.concatenate((height_series[::decimate_by],height_series[-1:]))
        llats = np.concatenate((llats[::decimate_by],llats[-1:]))
    poly_y = np.concatenate((height_series,np.zeros(height_series.shape)+bottom))
    poly_x = np.concatenate((llats,np.flip(llats)))
    mask_img = axb.fill(
        poly_x, poly_y, facecolor=colour, edgecolor='white',lw=0.02,zorder=lon_idx
    )
...
for lon_idx in range(359,3600,1):
    ...
    plot_layer(lon_idx,col,decimate_by=None)
