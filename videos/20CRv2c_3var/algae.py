...
parser.add_argument("--opdir", help="Directory for output files",
                    default="%s/images/20CRv2c_global_3var" % \
                                           os.getenv('SCRATCH'),
                    type=str,required=False)
parser.add_argument("--zfile", help="Noise pickle file name",
                    default="%s/images/20CRv2c_global_3var/z.pkl" % \
                                           os.getenv('SCRATCH'),
                    type=str,required=False)
...
algae_cm = matplotlib.colors.ListedColormap(list(reversed(cm_data)), name='algae_cm_')
...
precip.data = numpy.ma.array(precip.data,mask=precip.data<0.81)
cols=[]
for ci in range(100):
...
fig.savefig('%s/a_%04d%02d%02d%02d%02d.png' % (args.opdir,args.year,
                                             args.month,args.day,
                                             int(args.hour),
                                             int(args.hour%1*60)))
