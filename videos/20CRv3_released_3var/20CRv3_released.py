...
parser.add_argument("--opdir", help="Directory for output files",
                    default="%s/images/20CRv3_released_3var" % \
                                           os.getenv('SCRATCH'),
                    type=str,required=False)
parser.add_argument("--zfile", help="Noise pickle file name",
                    default="%s/images/20CRv3_released_3var/z.pkl" % \
                                           os.getenv('SCRATCH'),
                    type=str,required=False)
...
def load_around(var,dte,member=1,hours=5*24):
    tc=iris.Constraint(time=lambda cell: dte-datetime.timedelta(hours=hours) <= cell < dte+datetime.timedelta(hours=hours))
    file_name="%s/20CR/version_3/%04d/%s.%04d_mem%03d.nc" % (
                   os.getenv('SCRATCH'),dte.year,var,dte.year,member)
    f=iris.load_cube(file_name,tc)
...
recent_t=load_around('TMP2m',dte)                             
