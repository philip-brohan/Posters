...
Options<-WeatherMap.set.option(Options,'pole.lon',290)
Options<-WeatherMap.set.option(Options,'pole.lat',55)

Options<-WeatherMap.set.option(Options,'lat.min',-45)
Options<-WeatherMap.set.option(Options,'lat.max',45)
Options<-WeatherMap.set.option(Options,'lon.min',-85)
Options<-WeatherMap.set.option(Options,'lon.max',95)
Options$vp.lon.min<- -85
Options$vp.lon.max<-  95
...
recentre.obs<-function(obs,Options) {
...
plot.obs.coverage<-function(obs.new,obs.old,Options) {
  ...
  if(length(w.n)>0) {
     ...
     if(length(w.new>0)) {
        ...
        gp<-gpar(col='black',fill=Options$obs.colour,lwd=5)
        grid.points(x=unit(obs.plt$Longitude,'native'),
                    y=unit(obs.plt$Latitude,'native'),
   ...
   if(length(w.n)<length(idx.new)) {
        ...
        grid.points(x=unit(obs.plt$Longitude,'native'),
                    y=unit(obs.plt$Latitude,'native'),
                    size=unit(Options$obs.size*1.75,'native'),
                    pch=21,gp=gp)
   }
}
...
overlay.fog<-function(fog,Options) {
    ...
    fog.gsgrid<-fog
    fog.gsgrid$dimensions[[2]]<-list(type='lat',values=seq(Options$lat.max,
                                                           Options$lat.min,
                            (Options$lat.min-Options$lat.max)/(length(fog.sample[,1,1])-1)))
    fog.gsgrid$dimensions[[1]]<-list(type='lon',values=seq(Options$lon.min,
                                                           Options$lon.max,
                            (Options$lon.max-Options$lon.min)/(length(fog.sample[1,,1])-1)))
...
image.name<-sprintf("china.png",year,month,day,hour)
