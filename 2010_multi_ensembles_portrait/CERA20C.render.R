...
# The weather of December 2010 - in the CERA20C reanalyis
...
set.t2m.colour<-function(temperature,Trange=10) {
  ...
  n.colours<-11
  t.colours<-add.alpha(rev(brewer.pal(11,'RdBu')),1)
  s<-sign(temperature)
...
draw.streamlines<-function(s,Options) {
    ...
    u<-CERA20C.get.slice.at.hour('uwnd.10m',opt$year,opt$month,opt$day,opt$hour,member=opt$member)
    v<-CERA20C.get.slice.at.hour('vwnd.10m',opt$year,opt$month,opt$day,opt$hour,member=opt$member)
...
image.name<-sprintf("CERA20C_%02d.png",opt$member)
...
png(ifile.name,
  ...
  t2m<-CERA20C.get.slice.at.hour('air.2m',opt$year,opt$month,opt$day,opt$hour,member=opt$member)
  ...
  draw.grid(t2m,gc,set.t2m.colour,Options,grid.lwd=2.0,grid.lty=1)

  mslp<-CERA20C.get.slice.at.hour('prmsl',opt$year,opt$month,opt$day,opt$hour,member=opt$member)
  draw.pressure(mslp,Options)

  streamlines<-readRDS(sprintf("%s/CERA20C.streamlines.rd",Imagedir))
  draw.streamlines(streamlines,Options)
 
  prate<-CERA20C.get.slice.at.hour('prate',opt$year,opt$month,opt$day,opt$hour,member=opt$member)
  prate$data[]<-prate$data/3.6 # Convert to Kg/m/s
  WeatherMap.draw.precipitation(prate,Options)
  draw.grid(prate,gc,set.precip.colour,Options,grid.lwd=2.0,grid.lty=1)
