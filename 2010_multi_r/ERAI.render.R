...
# The weather of December 2010 - in the ERA-Interim reanalyis
...
draw.pressure<-function(mslp,Options,colour=c(0,0,0)) {
  ...
  if(!is.na(lines) && length(lines)>0) {
     for(i in seq(1,length(lines))) {
         ...
         res<-tryCatch({            grid.lines(x=unit(lines[[i]]$x,'native'),
...
draw.name<-function(Options) {
    ...
    xp<-unit(0.95,'npc')
    yp<-unit(0.98,'npc')    
    tg<-textGrob('ERA Interim',x=xp,y=yp,
                              hjust=1,vjust=0,
                              gp=label.gp)
   bg.gp<-gpar(col=rgb(1,1,1,0),fill=rgb(1,1,1,0.55))
   h<-heightDetails(tg)
...
image.name<-sprintf("ERAI.pdf",year,month,day,hour)
ifile.name<-sprintf("%s/%s",Imagedir,image.name)
 ...
 pdf(ifile.name,
  ...
  draw.grid(t2m,gi,set.t2m.colour,Options,grid.lwd=0.5,grid.lty=1)

  mslp<-ERAI.get.slice.at.hour('prmsl',opt$year,opt$month,opt$day,opt$hour)
  draw.pressure(mslp,Options)

  streamlines<-readRDS(sprintf("%s/ERAI.streamlines.rd",Imagedir))
  ...
  draw.grid(prate,gi,set.precip.colour,Options,grid.lwd=0.5,grid.lty=1)
