...
# Make the plot
pdf(file=sprintf("%s/FW_only_validation_18971128.pdf",Imagedir),
    ...
    pushViewport(viewport(width=1.0,height=1.0,x=0.0,y=0.0,
       ...
       pushViewport(plotViewport(margins=c(0,1,3,12)))
            ...
            for(s in seq(length(stations$X1897112818),1,-1)) {
              ...
              if(s%%2==0) {
                grid.polygon(x=unit(c(0,1,1,0),'npc'),
                             y=unit(c(ms-s-0.5,ms-s-0.5,
                                      ms-s+0.5,ms-s+0.5),'native'),
                             gp=gpar(col=rgb(0,0,0,0),fill=rgb(0,0,0,0.05)))
              }
            }
