...
plot.hour<-function(year,month,day,hour) {    
    ...
    # Mark the Fort William ob
         obs<-list(Latitude=station.lat,
                   Longitude=station.lon,
                   Label="Fort William",
                   Value=sprintf("%d",round(FW.data/100)))
    ...
    for(s in seq_along(stations$name)) {
       if(s %in% included) {
         obs<-list(Latitude=stations$latitude[s],
                   ...
                   Value=sprintf("%d",round(stations$X1897112818[s]/100)))
           ...
           # Plot the observed pressures
            ...
            for(s in seq(length(stations$X1897112818),1,-1)) {
              grid.lines(x=unit(stations$X1897112818[s],'native'),
                        y=unit(c(ms-s-0.3,ms-s+0.3),'native'),
                        gp=gp)
            }
         popViewport()
       popViewport()
